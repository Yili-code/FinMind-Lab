...
def test_backend():
    ...
    try:
        ...
        print(f"[OK] 根路徑測試: {response.status_code}")
    ...
    try:
        ...
        print(f"[OK] /api/hello 測試: {response.status_code}")
        print(f"  回應: {response.json()}\n")
    except Exception as e:
        print(f"[X] /api/hello 測試失敗: {e}\n")
        return False
    
    # 測試股票資訊 API
    try:
        response = requests.get(f"{base_url}/api/stock/info/2330", timeout=10)
        print(f"[OK] 股票資訊 API 測試: {response.status_code}")
